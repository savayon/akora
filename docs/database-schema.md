...
| `status` | enum | DEFAULT `'in_progress'` | 상태 (`in_progress`, `judging`, `completed`) |
| `required_jurors` | int | DEFAULT 5 | 토론을 종료하기 위해 필요한 판정 수 |
| `winner_id` | uuid | FK(`users.id`), NULL | 다수결 판정에 의해 승리한 참가자 ID |
| `created_at` | timestamptz | DEFAULT `now()` | 생성 일시 |

---

### 2.5. `judgments` (신규 - 배심원 판정)
토론 종료 후 승패를 결정하는 배심원 판정 결과입니다. 향후 AI 배심원 등 확장을 고려한 구조입니다.

| 컬럼명 | 데이터 타입 | 제약 조건 | 설명 |
|---|---|---|---|
| `id` | uuid | PK | 고유 식별자 |
| `debate_id` | uuid | FK(`debates.id`), NOT NULL | 판정이 이루어진 토론방 |
| `juror_id` | uuid | FK(`users.id`), NULL | 배심원 (사람일 경우 users.id) |
| `juror_type` | enum | DEFAULT `'human'` | 배심원 종류 (`'human'`, `'ai'`, `'expert'`) |
| `voted_for_id` | uuid | FK(`users.id`), NOT NULL | 승리자로 선택한 토론 참가자 ID |
| `reason` | text | NOT NULL | 판정 이유 |
| `created_at` | timestamptz | DEFAULT `now()` | 판정 시간 |
> UNIQUE(`debate_id`, `juror_id`) 제약조건으로 1인 1회 판정을 보장합니다.
